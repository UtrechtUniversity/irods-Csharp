...
namespace irods_Csharp
{
    internal abstract class IRodsMessage
    {
        ...
        {
            List<SqlResult_PI> results = new List<SqlResult_PI>();
    ...
    internal class GenQueryOut_PI : IRodsMessage
    {
        ...
        internal object Parse(Type type, IrodsSession session, Path home, Path path, Options.FileMode mode = Options.FileMode.ReadWrite, bool truncate = false)
        {
            if (type == typeof(Collection))
            {
                ...
                {
                    Collection collection = new Collection(new Path(SqlResult_PI[collectionNameColumn].value[i].Replace(home.ToString(), "")), int.Parse(SqlResult_PI[collectionIdColumn].value[i]), session.Collections);
            ...
            {
                List<DataObj> objects = new List<DataObj>();
            ...
            if (type == typeof(Meta))
            {
                ...
                for (int i = 0; i < rowCnt; i++)
                {
                    ...
                    Meta meta = new Meta(SqlResult_PI[metaNameColumn].value[i], SqlResult_PI[metaKeywordColumn].value[i], units);
                    objects[i] = meta;
                }
                return objects;
            }
    ...
    {
        public string arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9;
    ...
    internal class KeyValPair_PI : IRodsMessage
    {
        ...
        public override string ToString()
        {
            ...
            output += $"<ssLen>{ssLen}</ssLen>\n";
            if (keyWord != null) foreach (string keyword in keyWord) output += $"<keyWord>{keyword}</keyWord>\n";
            if (keyWord != svalue) foreach (string value in svalue) output += $"<svalue>{value}</svalue>\n";
            output += $"</{GetType().Name}>";
            return output;
        }
    }
    ...
    internal class InxIvalPair_PI : IRodsMessage
    {
        ...
        {
            string output = $"<{GetType().Name}>\n";
            output += $"<iiLen>{iiLen}</iiLen>\n";
            if (inx != null) foreach (int inx in inx) output += $"<inx>{inx}</inx>\n";
            if (ivalue != null) foreach (int ivalue in ivalue) output += $"<ivalue>{ivalue}</ivalue>\n";
            output += $"</{GetType().Name}>";
            return output;
        }
    }
    ...
    internal class InxValPair_PI : IRodsMessage
    {
        ...
        {
            string output = $"<{GetType().Name}>\n";
            output += $"<isLen>{isLen}</isLen>\n";
            if (inx != null) foreach (int inx in inx) output += $"<inx>{inx}</inx>\n";
            if (svalue != null) foreach (string svalue in svalue) output += $"<svalue>{svalue}</svalue>\n";
            output += $"</{GetType().Name}>";
            return output;
        }
    }
